...
def add_follow(email1, email2):
    DBconnect.exist(entity="Users", identificator="email", value=email1)
    DBconnect.exist(entity="Users", identificator="email", value=email2)

    if email1 == email2:
        raise Exception("User with email=" + email1 + " can't follow himself")

    follows = DBconnect.exec_query(
        'SELECT id FROM Followers WHERE follower = %s AND followee = %s', (email1, email2, )
    )

    if len(follows) == 0:
        DBconnect.exec_update('INSERT INTO Followers (follower, followee) VALUES (%s, %s)',
                              (email1, email2, ))
...
def followers_list(email, type, params):
    ...
    followers_ids_tuple = DBconnect.exec_query(query=query, params=(email, ))
